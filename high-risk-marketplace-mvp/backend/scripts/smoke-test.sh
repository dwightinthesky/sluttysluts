...
echo "[5/14] purchase click-based promotion"
...
echo "[6/14] consume one promotion click"
request POST "/v1/promotions/$PROMOTION_ID/consume-click" "buyer-demo" | python3 -m json.tool

echo "[7/14] inspect ranked listings"
request GET "/v1/listings?status=all" "buyer-demo" | python3 -m json.tool

echo "[8/14] place order (pending + hosted checkout)"
ORDER_RESPONSE="$(request POST "/v1/orders" "buyer-demo" "{\"buyerId\":\"buyer-demo\",\"listingId\":\"$LISTING_ID\",\"currency\":\"USD\",\"expectedTotalCharged\":126}")"
echo "$ORDER_RESPONSE" | python3 -m json.tool
ORDER_ID="$(echo "$ORDER_RESPONSE" | json_get "data['order']['id']")"
CHECKOUT_URL="$(echo "$ORDER_RESPONSE" | json_get "data['checkout']['checkoutUrl']")"
echo "Hosted checkout URL: $CHECKOUT_URL"

echo "[9/14] simulate payment webhook callback"
curl -sS -X POST "$BASE_URL/v1/payments/webhooks/high-risk" \
  -H "Content-Type: application/json" \
  -H "x-webhook-token: $WEBHOOK_TOKEN" \
  -d "{\"eventId\":\"evt-$ORDER_ID-paid\",\"orderId\":\"$ORDER_ID\",\"paymentStatus\":\"paid\",\"pspTxnId\":\"psp-$ORDER_ID\"}" | python3 -m json.tool

echo "[10/14] delivery + force release payout"
...
echo "[13/14] create chat thread + send message"
