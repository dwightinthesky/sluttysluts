      ...
      function parseWalletSummary(payload) {
        ...
        const reserveRows =
          (payload && payload.upcomingReserveReleases) ||
      ...
      function renderWalletSummaryDisplay() {
        ...
        if (el.walletReserveReleases) {
          ...
          if (!releases.length) {
            el.walletReserveReleases.innerHTML = `<div class="reserve-release-empty">${escapeHtml(
              t('walletReserveEmpty', 'No pending reserve releases.')
            )}</div>`;
          } else {
      ...
      function walletLedgerCodeClass(code) {
        ...
        if (normalized === 'reserve_hold') return 'reserve_hold';
        if (normalized === 'reserve_release') return 'reserve_release';
        if (normalized === 'withdrawal') return 'withdrawal';
        if (normalized === 'promo_fee') return 'promo_fee';
        if (normalized === 'debit') return 'debit';
        return 'credit';
      }

      function renderWalletLedger() {
        if (!el.walletLedgerBody) return;
        if (!Array.isArray(state.walletLedger) || !state.walletLedger.length) {
          el.walletLedgerBody.innerHTML = `
            <tr>
              <td colspan="4" class="wallet-ledger-empty">${escapeHtml(
                t('walletLedgerEmpty', 'No wallet ledger entries yet.')
              )}</td>
            </tr>
          `;
          return;
      ...
      function moderationRiskLabel(riskBand) {
        if (riskBand === 'high') return t('moderationRiskHigh', 'High Risk');
        if (riskBand === 'medium') return t('moderationRiskMedium', 'Medium Risk');
      ...
      function renderModerationQueue() {
        ...
        if (!el.moderationQueueGrid) return;
            ...
            return `
              <article class="moderation-item">
                ...
                <div class="moderation-foot">
                  <button class="moderation-btn reject" data-mod-action="reject" data-mod-id="${escapeHtml(
                    item.listingId || item.id || ''
                  )}">${escapeHtml(t('moderationReject', 'Reject'))}</button>
                  <button class="moderation-btn approve" data-mod-action="approve" data-mod-id="${escapeHtml(
                    item.listingId || item.id || ''
                  )}">${escapeHtml(t('moderationApprove', 'Approve'))}</button>
                </div>
              </article>
            `;
          })
